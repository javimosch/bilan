...
# 4. asset + checksum
cp bilan bilan-linux-x64
SHA=$(sha256sum bilan-linux-x64 | cut -d' ' -f1)
echo "✓ gate green · bilan-linux-x64 $(du -h bilan-linux-x64 | cut -f1) · sha256 $SHA"

if [ "$DRY" = "--dry-run" ]; then echo "(dry-run — no release created)"; rm -f bilan-linux-x64; exit 0; fi

# 5. cut the release
gh release create "$TAG" bilan-linux-x64 -R "$REPO" \
  --title "bilan $TAG" \
  --notes "Install: \`curl -fsSL https://raw.githubusercontent.com/$REPO/master/install.sh | sh\` · update: \`bilan update\`.

Prebuilt \`bilan-linux-x64\` (glibc ≥ 2.35, libsqlite3); other platforms auto-fall-back to source.

\`sha256(bilan-linux-x64) = $SHA\`"
rm -f bilan-linux-x64
echo "✓ released $TAG"
